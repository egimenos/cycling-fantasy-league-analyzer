...
def test_value_score_weights_by_year_and_type():
    ...
    results = [
        RiderResultDTO(
            rider_id=uuid4(), points=100, year=_year(0), race_type=RaceType.ONE_DAY
        ),
        RiderResultDTO(
            rider_id=uuid4(), points=100, year=_year(1), race_type=RaceType.ONE_DAY
        ),
        RiderResultDTO(
            rider_id=uuid4(), points=100, year=_year(2), race_type=RaceType.STAGE_RACE
        ),
        RiderResultDTO(
            rider_id=uuid4(), points=100, year=_year(3), race_type=RaceType.ONE_DAY
        ),
    ]

    total, value = calc.calculate(results, price=200, target_race_type=RaceType.ONE_DAY)
...
def test_value_score_zero_price():
    calc = ValueScoreCalculator()
    total, value = calc.calculate([], price=0, target_race_type=RaceType.ONE_DAY)
    assert total == 0
    assert value == 0.0
