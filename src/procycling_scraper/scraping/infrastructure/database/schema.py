...
classifications_table = Table(
    ...
    Column(
        "type",
        PgEnum(ClassificationType, name="classification_type_enum"),
        nullable=False,
    ),
    ...
    UniqueConstraint(
        "race_id", "type", "stage_number", name="uq_classification_in_race"
    ),
)
...
pcs_points_results_table = Table(
    ...
    Column("rider_id", UUID(as_uuid=True), ForeignKey("riders.id"), nullable=False),
    ...
    UniqueConstraint(
        "classification_id", "rider_id", name="uq_result_per_rider_per_classification"
    ),
)
