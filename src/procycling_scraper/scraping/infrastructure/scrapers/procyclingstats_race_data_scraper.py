...
class ProCyclingStatsRaceDataScraper(RaceDataScraper):
    ...
    def scrape(self, race_info: Tuple[str, RaceType]) -> ScrapedRaceData:
        ...
        try:
            ...
            race = Race(pcs_id=base_race_url_path, name="Scraping Failed", year=0, race_type=race_type)
    ...
    def _scrape_one_day_race(self, race_url: str, race_type: RaceType) -> ScrapedRaceData:
        ...
        if not soup:
            return ScrapedRaceData(race=race, riders=[])
        gc_classification, gc_riders = self._scrape_classification_table(soup, ClassificationType.GENERAL)
    ...
    def _scrape_stage_race(self, race_url: str, race_type: RaceType) -> ScrapedRaceData:
        ...
        if not classification_urls:
            print("WARN: Could not find classification URLs from select menu. Scraping current page as GC.")
            _, gc_riders = self._scrape_classification_table(soup, ClassificationType.GENERAL)
            ...
            for url_path, classification_type, stage_num in classification_urls:
                ...
                classification, riders = self._scrape_classification_table(page_soup, classification_type, stage_num)
    ...
    def _parse_select_menu_options(self, select_menu: Tag) -> List[Tuple[str, ClassificationType, Optional[int]]]:
        urls_to_scrape: List[Tuple[str, ClassificationType, Optional[int]]] = []
        for option in select_menu.find_all("option"):
            if not isinstance(option, Tag) or not option.has_attr("value"): continue
            value_attr = option["value"]
            if not isinstance(value_attr, str) or not value_attr: continue
            url_path, option_text = value_attr, option.text.lower()
            if "teams" in url_path or "youth" in url_path: continue
            ...
            if "stage-" in url_path and stage_num_match and "points" not in url_path and "kom" not in url_path:
                urls_to_scrape.append((url_path, ClassificationType.STAGE, int(stage_num_match.group(1))))
                continue
            if "points classification" in option_text:
                urls_to_scrape.append((url_path, ClassificationType.POINTS, None))
                ...
                urls_to_scrape.append((url_path, ClassificationType.GENERAL, None))
    ...
    def _scrape_race_details(self, url: str, race_type: RaceType) -> Tuple[Race, BeautifulSoup]:
        ...
        if not soup: raise ValueError(f"Could not fetch or parse page: {url}")
        title_tag = soup.find("h1")
        if not isinstance(title_tag, Tag): raise ValueError(f"Race title (h1) not found on page: {url}")
        title = title_tag.text.strip()
        year_match = re.search(r"^\d{4}", title)
        year = int(year_match.group(0)) if year_match else 0
        pcs_id = re.sub(r"/(gc|result|results|stage-\d+|points|kom).*", "", url).replace(f"{self._base_url}/", "")
    ...
    def _scrape_classification_table(self, soup: BeautifulSoup, classification_type: ClassificationType, stage_number: Optional[int] = None) -> Tuple[Classification, List[Rider]]:
        ...
        if not isinstance(table, Tag):
            print(f"WARN: Results table 'div.resTab:not(.hide) table.results' not found for {classification_type.value}")
            return Classification(classification_type, [], stage_number), []
        try:
            thead = table.find("thead")
            if not isinstance(thead, Tag): raise ValueError("Table head not found")
            header_tags = thead.find_all("th")
            team_headers, points_header, rider_header = ["Team", "Tm"], "Pnt", "Rider"
            headers = [h.text.strip() for h in header_tags]
            team_header_found = next((th for th in team_headers if th in headers), None)
            if not team_header_found: raise ValueError("Team header not found")
            header_map: Dict[str, int] = {"Rider": headers.index(rider_header), "Team": headers.index(team_header_found), "Pnt": headers.index(points_header)}
        except (ValueError, AttributeError):
            print(f"WARN: Could not find required headers in results table for {classification_type.value}. Skipping.")
            return Classification(classification_type, [], stage_number), []
        tbody = table.find("tbody")
        if not isinstance(tbody, Tag): return Classification(classification_type, [], stage_number), []
        for row_element in tbody.find_all("tr"):
            if not isinstance(row_element, Tag): continue
            parsed_data = self._parse_result_row(row_element, header_map)
            if parsed_data:
                rider, result_line = parsed_data
                results.append(result_line)
                found_riders.append(rider)
        classification = Classification(classification_type=classification_type, results=results, stage_number=stage_number)
    ...
    def _parse_result_row(self, row: Tag, header_map: Dict[str, int]) -> Optional[Tuple[Rider, ResultLine]]:
        if len(row.find_all("td")) <= max(header_map.values()): return None
        try:
            pcs_points_text = row.find_all("td")[header_map["Pnt"]].text.strip()
            pcs_points_str = pcs_points_text.split()[0] if pcs_points_text else "0"
            pcs_points = int(pcs_points_str)
            if pcs_points <= 0: return None
        except (ValueError, IndexError): return None
        rider_cell = row.find_all("td")[header_map["Rider"]]
        if not isinstance(rider_cell, Tag): return None
        rider_link = rider_cell.find("a")
        if not isinstance(rider_link, Tag): return None
        href_value = rider_link.get("href")
        if not isinstance(href_value, str): return None
        ...
        result_line = ResultLine(rider_pcs_id=rider_pcs_id, team_name=team_name, points=pcs_points)
