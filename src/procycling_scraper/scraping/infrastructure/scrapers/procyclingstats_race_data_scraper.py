...
if __name__ == "__main__":
    ...
    def print_scraped_data(title: str, data: ScrapedRaceData):
        ...
        print(
            f"Found {len(data.race.classifications)} classifications in total.")
        classification_details = {}
        if data.race.race_type == RaceType.STAGE_RACE:
            try:
                entry_url = f"{BASE_URL}/{data.race.pcs_id}/gc"
                entry_soup = data_scraper._get_page_soup(entry_url)
                if entry_soup:
                    urls = data_scraper._extract_classification_urls(
                        entry_soup)
                    for url, c_type, s_num in urls:
                        key = f"{c_type.value}-{s_num}" if s_num else c_type.value
                        classification_details[key] = f"{BASE_URL}/{url}"
            except Exception:
                pass
    ...
    stage_race_info = next(
        (r for r in races_to_scrape if 'tour-de-france' in r[0]), None)
    one_day_race_info = next(
        (r for r in races_to_scrape if 'paris-roubaix' in r[0] and 'u23' not in r[0]), None)
    data_scraper = ProCyclingStatsRaceDataScraper()
    if one_day_race_info:
        scraped_data_one_day = data_scraper.scrape(one_day_race_info)
        print_scraped_data("One-Day Race Test Results", scraped_data_one_day)
    if stage_race_info:
        scraped_data_stage = data_scraper.scrape(stage_race_info)
        print_scraped_data("Stage Race Test Results", scraped_data_stage)
