...
from procycling_scraper.scraping.domain.repositories.race_repository import (
    RaceRepository,
)
...
class PostgresRaceRepository(RaceRepository):
    ...
    def find_all_results_by_rider_ids(
        ...
        stmt = (
            ...
            .select_from(pcs_points_results_table)
            .join(
                classifications_table,
                pcs_points_results_table.c.classification_id
                == classifications_table.c.id,
            )
            .join(races_table, classifications_table.c.race_id == races_table.c.id)
            .where(pcs_points_results_table.c.rider_id.in_(rider_ids))
        )
        with self._engine.connect() as conn:
            db_results: List[Any] = list(conn.execute(stmt).fetchall())
        grouped_results: Dict[UUID, List[RiderResultDTO]] = {
    ...
    def _get_rider_id_map(self, conn: Connection, race: Race) -> Dict[str, UUID]:
        all_rider_pcs_ids = {
            r.rider_pcs_id for c in race.classifications for r in c.results
        }
    ...
    def _save_results(
        ...
        classification_db_id: UUID,
        rider_id_map: Dict[str, UUID],
    ):
        if not results:
            return
