    ...
    def execute(self, request_data: AnalysisRequestDTO) -> ApiResponsePayload:
        ...
        for cyclist_dto in request_data.cyclists:
            ...
            if best_match and best_match.id:
                matched_riders.append(MatchedRiderDTO(
                    api_data=cyclist_dto, db_rider=best_match))

        rider_ids_to_fetch: List[UUID] = [
            rider.db_rider.id for rider in matched_riders if rider.db_rider.id is not None]
        ...
        for rider_data in matched_riders:
            ...
            if db_rider.id:
                ...
                stage_race_points = sum(
                    r.points for r in rider_results if r.race_type == RaceType.STAGE_RACE)
